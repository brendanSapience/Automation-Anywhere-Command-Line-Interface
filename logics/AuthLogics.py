...
def checktoken(token):
    ...
    URL = DataUtils.GetUrl()+AUTH_TOKEN_URI
...
def login(url,login,password):
    ...
    isInError = AuthResponses.Process_Auth_Login_Response(response,url)

def logout():

    URL = DataUtils.GetUrl()+AUTH_LOGOUT_URI
    headers = {
        'Content-Type': "application/json",
        'cache-control': "no-cache",
        'X-Authorization': DataUtils.GetAuthToken()
    ...
    isInError = AuthResponses.Process_Auth_Logout_Response(response)
